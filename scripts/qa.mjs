...
if(command==='verify'){
  ...
  for(const [email,role,name,status] of [
    ...
    if(role==='practitioner'){
      ...
      if(!found.data){
        const inserted=await client.from('practitioners').insert({user_id:user.id,name,slug:email.split('@')[0]+'-qa',status,bio:'Profil fictif pour les tests de bout en bout.',languages:['fr'],contact:{email}}).select('id').single();if(inserted.error)throw inserted.error;
...
}else throw new Error('Usage: node scripts/qa.mjs seed|verify|serve|build|start');
