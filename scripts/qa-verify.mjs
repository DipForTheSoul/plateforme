...
export async function verifyLocal(config,password){
  ...
  try{
    ...
    const practitioner=ok(await service.from('practitioners').insert({user_id:userId,name:'QA temporaire',slug:`qa-${run}`,status:'approved',languages:['fr']}).select('id').single());
    ...
    ok(await anon.from('contacts').insert({email:contactEmail,consent:true,source:'qa',opt_in_at:new Date().toISOString()}));
    check('inscription newsletter publique enregistrée',ok(await service.from('contacts').select('consent').eq('email',contactEmail).single()).consent===true);
    check('contacts privés invisibles aux visiteurs',ok(await anon.from('contacts').select('id').eq('email',contactEmail)).length===0);
    check('doublon newsletter identifié sans exposer le contact',(await anon.from('contacts').insert({email:contactEmail,consent:true})).error?.code==='23505');
