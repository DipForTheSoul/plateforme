...
describe('Validation des expériences',()=>{
  function form() { const fd=new FormData(); Object.entries({title:'Test',description:'Description de test suffisamment longue.',category_ids:'10000000-0000-4000-8000-000000000001',start_date:'2026-10-01T11:00',languages:'fr',price_mode:'flexible',duration_minutes:'1.5'}).forEach(([k,v])=>fd.set(k,v)); return fd; }
  it('additionne les sélecteurs heures et minutes',()=>{const fd=form();fd.delete('duration_minutes');fd.set('duration_hours','1');fd.set('duration_minute_part','35');const result=parseEventForm(fd);expect(result.success && result.data.duration_minutes).toBe(95);});
  it('accepte des minutes seules',()=>{const fd=form();fd.delete('duration_minutes');fd.set('duration_hours','0');fd.set('duration_minute_part','45');const result=parseEventForm(fd);expect(result.success && result.data.duration_minutes).toBe(45);});
  it('refuse une valeur hors des bornes des sélecteurs',()=>{const fd=form();fd.delete('duration_minutes');fd.set('duration_hours','24');fd.set('duration_minute_part','0');expect(parseEventForm(fd).success).toBe(false);});
  it('continue de lire les anciennes durées décimales',()=>{const fd=form();fd.set('duration_minutes','1,5');const result=parseEventForm(fd);expect(result.success && result.data.duration_minutes).toBe(90);});
  it('force la durée à null sur plusieurs jours',()=>{const fd=form();fd.delete('duration_minutes');fd.set('duration_hours','1');fd.set('duration_minute_part','35');fd.set('end_date','2026-10-02T16:00');const result=parseEventForm(fd);expect(result.success && result.data.duration_minutes).toBe(null);});
