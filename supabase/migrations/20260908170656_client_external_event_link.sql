-- Dedicated optional link, copied atomically to every occurrence.
begin;
alter table public.events add column external_url text;
alter table public.events add constraint events_external_url_check check (
  external_url is null or (
    length(external_url) <= 2048 and
    external_url ~* '^https?://[^[:space:]/?#@]+\.[^[:space:]/?#@]+([/?#][^[:space:]]*)?$'
  )
);
...
begin
  ...
  if not public.is_admin() and auth.uid() is not null then
    ...
    if (new.title,new.description,new.category_id,new.venue_id,new.start_date,new.end_date,
        new.duration_minutes,new.price,new.languages,new.included,new.to_bring,new.video_url,new.external_url,new.images,new.recurrence,new.recurrence_count)
      is distinct from (old.title,old.description,old.category_id,old.venue_id,old.start_date,old.end_date,
        old.duration_minutes,old.price,old.languages,old.included,old.to_bring,old.video_url,old.external_url,old.images,old.recurrence,old.recurrence_count) then
...
begin
  ...
  if p_event_id is null then
    ...
    insert into public.events(title, slug, description, practitioner_id, category_id, venue_id, start_date, end_date,
      recurrence, recurrence_count, duration_minutes, price, languages, included, to_bring, video_url, external_url, images, status, submission_id)
    values (p_input->>'title', 'experience-' || gen_random_uuid()::text, p_input->>'description', owner_id, cats[1], (p_input->>'venue_id')::uuid,
      ...
      (p_input->>'duration_minutes')::integer, (p_input->>'price')::numeric, langs, p_input->>'included', p_input->>'to_bring', p_input->>'video_url', p_input->>'external_url', photos, state, p_submission_id)
  ...
  if old_event.parent_event_id is null then
    ...
    if schedule_changed then
      for item in select value from jsonb_array_elements(planned) loop
        ...
        if child_id is null then
          insert into public.events(title,slug,description,practitioner_id,category_id,venue_id,start_date,end_date,parent_event_id,duration_minutes,price,languages,included,to_bring,video_url,external_url,images,status)
          values(saved.title,'experience-' || gen_random_uuid()::text,saved.description,owner_id,cats[1],saved.venue_id,(item->>'start_date')::timestamptz,(item->>'end_date')::timestamptz,saved.id,saved.duration_minutes,saved.price,langs,saved.included,saved.to_bring,saved.video_url,saved.external_url,photos,state)
    ...
    for child in select * from public.events where parent_event_id=saved.id loop
      update public.events set title=saved.title,description=saved.description,category_id=cats[1],venue_id=saved.venue_id,
        duration_minutes=saved.duration_minutes,price=saved.price,languages=langs,included=saved.included,to_bring=saved.to_bring,video_url=saved.video_url,external_url=saved.external_url,images=photos,status=state
        where id=child.id;
