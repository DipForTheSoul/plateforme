-- Expand only the serialized-description allowance. No rows or publication states are modified.
-- Visible text remains limited to 8,000 characters by the application validator.
begin;
...
begin
  ...
  if coalesce(length(btrim(p_input->>'title')),0) not between 3 and 140
    or coalesce(length(btrim(p_input->>'description')),0) not between 20 and 16000
