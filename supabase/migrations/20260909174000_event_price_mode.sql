-- Additive pricing modes. No deletion, reseeding or conversion of old zero prices to free.
begin;
alter table public.events add column price_mode text;
update public.events set price_mode = case when price is null then 'unspecified' when price=0 then 'flexible' else 'fixed' end;
alter table public.events alter column price_mode set not null;
alter table public.events add constraint events_price_mode_check check (
  (price_mode='fixed' and price is not null and price>0)
  or (price_mode in ('free','flexible') and price is not null and price=0)
  or (price_mode='unspecified' and price is null)
);

-- Older direct inserts still retain their previous semantics.
create function public.default_event_price_mode() returns trigger
language plpgsql set search_path = '' as $$
begin
  if new.price_mode is null then
    new.price_mode := case when new.price is null then 'unspecified' when new.price=0 then 'flexible' else 'fixed' end;
  end if;
  return new;
end $$;
create trigger event_default_price_mode before insert on public.events
for each row execute function public.default_event_price_mode();
...
begin
  ...
  if not public.is_admin() and auth.uid() is not null then
    ...
    if (new.title,new.description,new.category_id,new.venue_id,new.start_date,new.end_date,
        new.duration_minutes,new.price,new.price_mode,new.languages,new.included,new.to_bring,new.video_url,new.external_url,new.images,new.recurrence,new.recurrence_count)
      is distinct from (old.title,old.description,old.category_id,old.venue_id,old.start_date,old.end_date,
        old.duration_minutes,old.price,old.price_mode,old.languages,old.included,old.to_bring,old.video_url,old.external_url,old.images,old.recurrence,old.recurrence_count) then
...
declare
  ...
  state text;
  pricing text;
  amount numeric := (p_input->>'price')::numeric;
begin
  ...
  -- Compatibility for requests from the previous deployed version; new forms
  -- explicitly supply a mode. Never infer 'free' from zero or a missing price.
  pricing := coalesce(p_input->>'price_mode', case
    when p_event_id is not null and amount is not distinct from old_event.price then old_event.price_mode
    when amount is null then 'unspecified' when amount=0 then 'flexible' else 'fixed' end);
  if pricing not in ('fixed','free','flexible','unspecified')
    or (pricing='fixed' and (amount is null or amount<=0))
    or (pricing in ('free','flexible') and amount is distinct from 0)
    or (pricing='unspecified' and amount is not null) then
  ...
  if p_event_id is null then
    ...
    insert into public.events(title, slug, description, practitioner_id, category_id, venue_id, start_date, end_date,
      recurrence, recurrence_count, duration_minutes, price, price_mode, languages, included, to_bring, video_url, external_url, images, status, submission_id)
    values (p_input->>'title', 'experience-' || gen_random_uuid()::text, p_input->>'description', owner_id, cats[1], (p_input->>'venue_id')::uuid,
      ...
      (p_input->>'duration_minutes')::integer, (p_input->>'price')::numeric, pricing, langs, p_input->>'included', p_input->>'to_bring', p_input->>'video_url', p_input->>'external_url', photos, state, p_submission_id)
    ...
    update public.events set title=p_input->>'title', description=p_input->>'description', category_id=cats[1], venue_id=(p_input->>'venue_id')::uuid,
      ...
      duration_minutes=(p_input->>'duration_minutes')::integer, price=(p_input->>'price')::numeric, price_mode=pricing, languages=langs,
      included=p_input->>'included', to_bring=p_input->>'to_bring', video_url=p_input->>'video_url', external_url=case when p_input ? 'external_url' then p_input->>'external_url' else old_event.external_url end, images=photos, status=state
  ...
  if old_event.parent_event_id is null then
    ...
    if schedule_changed then
      for item in select value from jsonb_array_elements(planned) loop
        ...
        if child_id is null then
          insert into public.events(title,slug,description,practitioner_id,category_id,venue_id,start_date,end_date,parent_event_id,duration_minutes,price,price_mode,languages,included,to_bring,video_url,external_url,images,status)
          values(saved.title,'experience-' || gen_random_uuid()::text,saved.description,owner_id,cats[1],saved.venue_id,(item->>'start_date')::timestamptz,(item->>'end_date')::timestamptz,saved.id,saved.duration_minutes,saved.price,saved.price_mode,langs,saved.included,saved.to_bring,saved.video_url,saved.external_url,photos,state)
    ...
    for child in select * from public.events where parent_event_id=saved.id loop
      update public.events set title=saved.title,description=saved.description,category_id=cats[1],venue_id=saved.venue_id,
        duration_minutes=saved.duration_minutes,price=saved.price,price_mode=saved.price_mode,languages=langs,included=saved.included,to_bring=saved.to_bring,video_url=saved.video_url,external_url=saved.external_url,images=photos,status=state
        where id=child.id;
