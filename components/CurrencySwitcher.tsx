...
export function CurrencySwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`inline-flex rounded-full border border-soul-bronze/30 bg-white font-semibold ${compact ? "p-0.5 text-xs" : "p-0.5 text-sm"}`}>
      {options.map(({ code, symbol }) => (
        <button
          ...
          aria-label={code}
          title={code}
          className={`rounded-full transition ${compact ? "px-1.5 py-0.5" : "px-3 py-1"} ${
            currency === code
